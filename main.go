...
func main() {
	...
	// TODO look recursively upwards for .gitem.yml/.gitem.json ?
	err := g.LoadConfig("example.yml")

	if err != nil {
		g.Error.Fprintln(os.Stderr, err)
		os.Exit(1)
		return
	}

	if g.config.Debug {
		g.Info.Fprintln(os.Stdout, g.config.Repos)
	}
